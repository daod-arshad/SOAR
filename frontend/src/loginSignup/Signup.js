...
export default function Signup() {
  ...
  const reg = /^\w+([\.-]?\w+)@\w+([\.-]?\w+)(\.\w{2,3})+$/;  //email
  const regp =/^(?=.\d)(?=.[a-z])(?=.*[A-Z]).{8,32}$/; //password
  ...
  const save = (e) => {
    ...
    if(info.Username.length===0 || info.Password.length===0 || !(info.Password.length<=8)
       || info.Fullname.length===0 || info.Email.length===0
      || info.Designation.length===0 || info.ConfirmPass.length===0 || reg.test(info.Email)===false
      || !(info.Password===info.ConfirmPass)){
      setErrormsg(true);
  }
  if (info.Username&&info.Password&&info.Email&&info.Fullname&&info.Designation&&info.ConfirmPass&&(reg.test(info.Email)===true)&&info.Password.length>8
  &&info.Password===info.ConfirmPass){
        ...
        <form className="formFields">
          ...
          <div className="formField">
            ...
            <input
              ...
              <div style={{height: '5px'}}>
                {error&&!(info.Email) ?
                  <div className='label'>Email Field can't be empty</div>: reg.test(info.Email)===false
                  ?<div className='label'>Email Field is Invalid</div>:""}</div>
          ...
          <div className="formField">
            ...
            <input
              ...
              <div style={{height: '5px'}}>
                  ...
                  <div className='label'>Password can't be empty</div>: !(info.Password.length>8)?
                  <div className='label'>Password length must be greater than 8</div>:""}</div>
          </div>
          ...
          <div className="formField">
            ...
            <input
              ...
              <div style={{height: '5px'}}>
                  ...
                  <div className='label'>Confirm Password can't be empty</div>: !(info.Password===info.ConfirmPass)?
                  <div className='label'>Doesn't match with password</div>:""}</div>
          </div>
